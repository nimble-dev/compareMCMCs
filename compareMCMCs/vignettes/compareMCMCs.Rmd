...
For both of these metrics, one may take interest in different components of computation time.  `compareMCMCs` records time in three components: setup time, burnin time, and post-burnin time.  The samples that are saved for analysis are the post-burnin samples.  Sampling time is defined as the sum of burnin and post-burnin time.  Setup time includes any steps prior to sampling.  It is possible to control which time is used for calculating MCMC efficiency and pace.  The default is sampling time.  
...
It is also possible to modify the samples by directly manipulating the `samples` element of an `MCMCresult` object.
...
* timing: table of burnin, post-burnin, and total sampling times.
...
* `MCMCinfo`: The element of `externalMCMCinfo` named to match this MCMC plugin.
