...
#' 
#' For an example, see `compareMCMCs:::MCMCdef_jags`.
#' 
...
#' 
#' These functions are normally called from \code{\link{compareMCMCs}}, which passes
#' its arguments or elements extracted from its arguments to these functions.
#' 
#' @param MCMCinfo The named element of `externalMCMCinfo` argument to \code{\link{compareMCMCs}} 
#' that matches a particular MCMC.  ("External" refers to any MCMC that is not internal to `nimble`.)
#' @param MCMCcontrol The `MCMCcontrol` argument to \code{\link{compareMCMCs}}, 
#' with the `seed` argument added as a list element if it was provided.
#' @param monitorInfo A list with elements `monitors` and `monitorVars`, providing 
#' two formats of information on model parameters for which MCMC output should be recorded.
#' @param modelInfo The `modelInfo` argument to \code{\link{compareMCMCs}}
#' 
#' @details 
...
#' 
#' MCMCs in `nimble` are run from `runNIMBLE`.  This uses a different system because
#' there may be multiple nimble MCMC configurations for one model.
#' 
#' `MCMCdef_dummy` does not run a real MCMC.  It provides a quick way to generate 
#' MCMC-formatted output for testing other parts of this package.
#' 
#' `MCMCdef_jags` runs JAGS via package `rjags`.  It uses model information from `modelInfo`.  
#' It does not use `MCMCinfo`.
#' 
#' `MCMCdef_stan` runs Stan via package `rstan`.  It does not use `modelInfo`.  It accepts the
#' following elements of the `MCMCinfo` list:
#' 
